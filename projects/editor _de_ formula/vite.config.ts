...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    /*
     * Porta na convenção das outras telas do Weknow: 517x no dev e 417x no
     * preview, uma faixa por projeto. 5173 é a análise avançada, 5174 o ASK,
     * 5176 a documentação do design system; esta tela fica com 5181.
     *
     * O 8443 que veio do Figma Make saiu junto com o `strictPort`: ele existia
     * porque a hospedagem do Make injeta `PORT` e não aceita o servidor pular
     * para outra porta. Fora de lá, isso só fazia o `npm run dev` morrer
     * quando a porta estava ocupada.
     */
    server: {
      host: '0.0.0.0',
      port: 5181,
      /* Libera o host do Tailscale. O ponto inicial casa qualquer subdomínio
         de ts.net, então continua valendo se a máquina mudar de nome. */
      allowedHosts: ['.ts.net'],
      watch: { ignored: ['**/.figma/**'] },
    },
    preview: {
